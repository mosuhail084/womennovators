        ...
        <div class="card">
          <div class="card-header pb-0">
            <div class="d-flex justify-content-between">
              <h4 class="card-title mg-b-0 mt-2"> We Pitch Event List</h4>
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table text-md-nowrap" id="example1">
                ...
                <tbody>
                  @foreach ($we_pitch_events as $event)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                      <td>{{ $event->we_pitch_title ?? '-----' }}</td>
                      <td>{{ $event->community_name ?? '-----' }}</td>
                      <td>{{ $event->we_pitch_mode ?? '-----' }}</td>
                      <td> {{ Carbon\Carbon::parse($event->we_pitch_start_date)->format('d/m/Y') ?? '-----' }} </td>
                      <td> {{ Carbon\Carbon::parse($event->we_pitch_end_date)->format('d/m/Y') ?? '-----' }} </td>
                      ...
                      <td>
                        <a href="{{ route('admin.we_pitch_edit_form', $event->id) }}"
                            class="btn ripple btn-primary text-white btn-icon"><i class="fe fe-edit"></i></a>
                      </td>
                      <td>
                          <a href="{{ url('/admin/we_pitch/show/' . $event->id) }}"

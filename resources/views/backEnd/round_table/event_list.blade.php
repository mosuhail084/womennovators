        ...
        <div class="card">
          <div class="card-header pb-0">
            <div class="d-flex justify-content-between">
              <h4 class="card-title mg-b-0 mt-2"> Round Table Event List</h4>
          ...
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table class="table text-md-nowrap" id="example1">
                ...
                <tbody>
                  @foreach ($round_table_events as $event)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                      <td>{{ $event->round_table_title ?? '-----' }}</td>
                      <td>{{ $event->community_name ?? '-----' }}</td>
                      <td>{{ $event->round_table_mode ?? '-----' }}</td>
                      <td> {{ Carbon\Carbon::parse($event->round_table_start_date)->format('d/m/Y') ?? '-----' }} </td>
                      <td> {{ Carbon\Carbon::parse($event->round_table_end_date)->format('d/m/Y') ?? '-----' }} </td>
                      ...
                      <td>
                        ...
                        @else
                          <span class="badge badge-danger-gradient">Deactive</span>
                        @endif
                      </td>

                      <td>
                        <a href="{{ route('admin.round_table_edit_form', $event->id) }}"
                            class="btn ripple btn-primary text-white btn-icon"><i class="fe fe-edit"></i></a>
                      </td>
                      <td>
                          <a href="{{ url('/admin/round_table/show/' . $event->id) }}"
